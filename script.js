...
class App {
  ...
  _hideForm() {
    ...
    // Insert the form at the top of the list
  ...
  _setLocalStorage() {
    const simplifiedWorkouts = this.#workouts.map(workout => ({
      type: workout.type,
    ...
    localStorage.setItem('workouts', JSON.stringify(simplifiedWorkouts));
  }
  ...
  _getLocalStorage() {
    ...
    if (storedData) {
      this.#workouts = storedData.map(data => {
